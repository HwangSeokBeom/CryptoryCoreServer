...
function resolveBaseUrl() {
  if (process.env.SMOKE_BASE_URL) {
    return process.env.SMOKE_BASE_URL.replace(/\/$/g, '');
  }
...
function bucketPointCount(pointCount) {
...
async function main() {
  ...
  for (const target of TARGETS) {
    ...
    for (const item of items) {
      ...
      pointCountDistribution[bucketPointCount(pointCount)] += 1;
    ...
    const row = {
      ...
      graphDisplayAllowedCount: items.filter((item) => item.graphDisplayAllowed === true).length,
      displayable24Count: items.filter((item) => item.graphDisplayAllowed === true && Number(item.sparklinePointCount ?? 0) >= 24).length,
      ...
      tickerRingBufferCount: summary.tickerRingBuffer ?? items.filter((item) => item.sparklineSource === 'ticker_ring_buffer').length,
